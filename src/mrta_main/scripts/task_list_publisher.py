...
def handle_comd(msg):
    ...
    global COMD_MSG
    COMD_MSG = msg
    cmd_check = validate_command(msg.header.frame_id)
...
prev_comd_msg_seq = 0
def check_new_comd(comd_msg):
    global prev_comd_msg_seq
    if comd_msg.header.seq != prev_comd_msg_seq:
        result = True
    else:
        result = False
...
def main_loop():
    ...
    #subscribers
    rospy.Subscriber(T_COMD, PoseStamped, handle_comd)
